...
type ResourceLocations map[string]*Location

type Module map[string]*hcl.File

func ParseResourceLocations(fs fs.FS, rootDir string) (ResourceLocations, error) {
	return parseDir(fs, rootDir, "")
}

// parseResourceLocations recursively parses all resource and module source locations.
func parseDir(fs fs.FS, dir string, modName string) (ResourceLocations, error) {
	mod, err := parseModuleDir(fs, dir)
	if err != nil {
		return nil, fmt.Errorf("parse %s: %w", dir, err)
	}

	resourceLocations := make(ResourceLocations)
	for name, file := range mod {
		rl, err := parseBlocks(fs, file, name, dir, modName)
		if err != nil {
...
func parseBlocks(fs fs.FS, file *hcl.File, fileName, dir, modAddr string) (ResourceLocations, error) {
...
func parseModuleDir(files fs.FS, dir string) (Module, error) {
