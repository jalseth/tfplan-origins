...
func TestParseResourceLocations(t *testing.T) {
	t.Parallel()
  ...
  source = "git+ssh://......"
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got, err := ParseResourceLocations(files, tc.rootDir)
			if gotErr := err != nil; gotErr != tc.wantErr {
				t.Fatalf("ParseResourceLocations() error = %v, want %v", err, tc.wantErr)
			}
			if tc.wantErr {
				return
			}
			if diff := cmp.Diff(got, tc.want); diff != "" {
				t.Errorf("ParseResourceLocations() produced unexpected diff:\n%s", diff)
			}
		})
	}
}
